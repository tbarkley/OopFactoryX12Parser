...
>
	...
	<xsl:template name="Member">
		...
		<xsl:if test="count($Loop/DMG)>0">
			...
			<xsl:for-each select="$Loop/../TRN">
				<TraceNumber>
					<xsl:call-template name="TraceNumber">
						<xsl:with-param name="TRN" select="."/>
					</xsl:call-template>
				</TraceNumber>
			</xsl:for-each>
		<Name>
		...
		<xsl:if test="count($Loop/REF)>0">
			<xsl:for-each select="$Loop/REF">
				<Identification>
					<xsl:call-template name="Identification">
						<xsl:with-param name="REF" select="."/>
					</xsl:call-template>
	...
	<xsl:template name="TraceNumber">
		...
		<xsl:attribute name="OriginatingCompanyIdentifier">
			<xsl:value-of select="TRN03"/>
		</xsl:attribute>
		<Type>
			<xsl:attribute name="Code">
				<xsl:value-of select="TRN01"/>
			</xsl:attribute>
			<xsl:value-of select="TRN01/comment()"/>
		</Type>
	</xsl:template>
